...
class Ball:
    def __init__(self, wn, jogo, padle_left, padle_right, color = 'white', ):
        self.ball = Turtle()
        self.ball.speed()
    ...
    def check_colison_padle(self):
        speed = 0.5
        if (self.xcor > self.padle_right.xcor - 20) and ( self.ycor <= self.padle_right.ycor + 50 and self.ycor >= self.padle_right.ycor - 50 ):
            self.ball.dx *= -1
            self.ball.goto(self.padle_right.xcor -25, self.ycor)
            self.ball.dx -= speed
            self.ball.dy -= speed
            
        if ( self.xcor <= self.padle_left.xcor + 20  ) and ( self.ycor <= self.padle_left.ycor + 50 and self.ycor >= self.padle_left.ycor - 50 ):
            self.ball.dx *= -1
